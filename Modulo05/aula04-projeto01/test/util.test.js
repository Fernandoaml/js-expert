...
describe("Util", () => {
  it("#evaluateRegex should throw an error using an unsafe regex", () => {
    ...
    /*
      time \
      node --eval "/^([a-z|A-Z|0-9]+\s?)+$/.test('eaaae man como vai voce e
       como vai?') && console.log('Não vai subir....')"
    */
    expect(() =>
      evaluateRegex(unsafeRegex).to.throw(
        InvalidRegexError,
        `This ${unsafeRegex} is a unsafe dude!`
      )
    );
  });
